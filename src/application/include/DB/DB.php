...
class DB{
	...
	public static function Base($B="", $T=""){
	    self::$conn['base'] = !empty($B) ? $B : DBBASE;
		self::$conn['table'] = $T;
		self::connMain();
	...
	public static function QUERY($_Q){
		$i=0;
		if(self::$connect){
			@$data=mysql_query($_Q, self::$connect);
	...
	public static function SET($Table,$Where="",$DES="",$LIMIT="",$SET="*"){
		$i=0;
		if(self::$connect){
			if(!empty($Where)){
				$W="WHERE $Where";
		    ...
		    @$data=mysql_query("SELECT $SET FROM $T $W $D $L",self::$connect);
			while(@$DB_ar=mysql_fetch_array($data,MYSQL_ASSOC)){
				$_DB[$i]=$DB_ar;
				$i++;
			}
	...
	public static function IN($T,$P,$V){
		if(self::$connect){
		    ...
		    $query   = mysql_query("INSERT INTO $T ($P) VALUES ($V)",self::$connect);
